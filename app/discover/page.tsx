...
import { useGetFeed, useGetFeedStories, useLikeFeedPost, useSaveFeedPost } from "@workspace/api-client-react";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Button } from "@/components/ui/button";
import { Heart, MessageCircle, Share2, Bookmark, ShoppingBag, Plus, MoreHorizontal } from "lucide-react";
...
export default function Discover() {
  ...
  const { data: stories, isLoading: isLoadingStories } = useGetFeedStories({
    query: { queryKey: ["/api/feed/stories"] }
  });

  const { data: feed, isLoading: isLoadingFeed } = useGetFeed({}, {
    query: { queryKey: ["/api/feed"] }
  });
