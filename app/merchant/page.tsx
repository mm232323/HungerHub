...
import { useGetMerchant, useGetMerchantProducts, useGetMerchantReviews, useFollowMerchant } from "@workspace/api-client-react";
...
export default function MerchantProfile({ id }: { id: string }) {
  const merchantId = parseInt(id);
  const { toast } = useToast();

  const { data: merchant, isLoading: isLoadingMerchant } = useGetMerchant(merchantId, {
    query: { enabled: !!merchantId, queryKey: ["/api/merchants", merchantId] }
  });

  const { data: products, isLoading: isLoadingProducts } = useGetMerchantProducts(merchantId, {
    query: { enabled: !!merchantId, queryKey: ["/api/merchants", merchantId, "products"] }
  });

  const { data: reviews, isLoading: isLoadingReviews } = useGetMerchantReviews(merchantId, {
    query: { enabled: !!merchantId, queryKey: ["/api/merchants", merchantId, "reviews"] }
  });
