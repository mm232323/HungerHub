...
import { ChevronRight, Clock, Star, TrendingUp, MapPin } from "lucide-react";
...
function TrendingMerchants({
  trendingMerchants: initialMerchants,
}: {
  ...
  useEffect(() => {
    if (location && !error) {
      ...
      // We will need to implement getTrendingMerchants in client-api.ts or just fetch from the API route
    ...
    <section className="py-8 px-4 container mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          {location && !error ? <MapPin className="h-5 w-5 text-primary" /> : <TrendingUp className="h-5 w-5 text-primary" />}
          <h2 className="text-2xl font-bold">
            {location && !error ? t("trendingMerchants.nearbyTitle") : t("trendingMerchants.title")}
          </h2>
        </div>
        <Link
          href="/discover"
      ...
      {isFetchingNearby ? (
        <div className="text-sm text-muted-foreground">{t("trendingMerchants.findingNearby")}</div>
      ) : merchants.length === 0 ? (
        <div className="text-sm text-muted-foreground">{t("trendingMerchants.noMerchants")}</div>
      ) : (
